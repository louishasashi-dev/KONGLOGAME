...
function initAssetGrids() {
  Object.keys(assetData).forEach((category) => {
    ...
    assetData[category].forEach((asset, index) => {
      ...
      let imageHtml = asset.image
        ? `<img src="${asset.image}" alt="${asset.name}" class="asset-image" onerror="this.style.display='none'">`
        : "";
...
function getAssetIncomeText(category, asset) {
  ...
  if (category === "industry" || category === "business")
    return `Income: $${formatNumberShort(asset.income)}/sec`;
...
// Convert ALL USD to IDR (All In)
function convertAllUsdToRupiah() {
  if (gameState.money > 0) {
    const usdAmount = gameState.money;
    gameState.money = 0;
    gameState.rupiahWallet += usdAmount * USD_TO_IDR;
    document.getElementById("usdAmount").value = "";
    updateUI();
    alert(
      `Berhasil menukar ${formatMoney(usdAmount)} USD menjadi ${formatMoney(usdAmount * USD_TO_IDR, "IDR")}`,
    );
  } else {
    alert("Saldo USD kosong!");
  }
}

// Convert ALL IDR to USD (All In)
function convertAllIdrToUsd() {
  if (gameState.rupiahWallet > 0) {
    const idrAmount = gameState.rupiahWallet;
    gameState.rupiahWallet = 0;
    gameState.money += idrAmount / USD_TO_IDR;
    document.getElementById("idrAmount").value = "";
    updateUI();
    alert(
      `Berhasil menukar ${formatMoney(idrAmount, "IDR")} menjadi ${formatMoney(idrAmount / USD_TO_IDR)} USD`,
    );
  } else {
    alert("Saldo IDR kosong!");
  }
}
...
document
  ...
  .addEventListener("click", () => statsPanel.classList.remove("active"));
...
document
  .getElementById("resetModalBtn")
  .addEventListener("click", () => resetModal.classList.add("show"));
document
  .getElementById("cancelResetBtn")
  .addEventListener("click", () => resetModal.classList.remove("show"));
